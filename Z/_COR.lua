...
sublimation_box0 = texts.new('${state}',{text={font='メイリオ', size=8}, pos={x=323, y=21}, padding = 2})
sublimation_box1 = texts.new('${state}',{text={font='メイリオ', size=8}, pos={x=408, y=21}, padding = 2})
sublimation_box2 = texts.new('${state}',{text={font='メイリオ', size=8}, pos={x=495, y=21}, padding = 2})
sublimation_box3 = texts.new('${state}',{text={font='メイリオ', size=8}, pos={x=580, y=21}, padding = 2})
sublimation_box4 = texts.new('${state}',{text={font='メイリオ', size=8}, pos={x=667, y=21}, padding = 2})
sublimation_box5 = texts.new('${state}',{text={font='メイリオ', size=8}, pos={x=753, y=21}, padding = 2})
--sublimation_box6 = texts.new('${state}',{text={font='メイリオ', size=8}, pos={x=837, y=21}, padding = 2})
-- sublimation_box7 = texts.new('${state}',{text={font='メイリオ', size=8}, pos={x=837, y=21}, padding = 2})
-- sublimation_box8 = texts.new('${state}',{text={font='メイリオ', size=8}, pos={x=925, y=21}, padding = 2})
sublimation_box0:show()
sublimation_box1:show()
sublimation_box2:show()
sublimation_box3:show()
sublimation_box4:show()
sublimation_box5:show()
--sublimation_box6:show()
-- sublimation_box7:show()
-- sublimation_box8:show()
...
function user_setup()
    ...
    state.RangedWeapons = M{['description']='Use Ranged Weapon', 'TPBonus', 'DeathPenalty', 'Fomalhaut', 'Armageddon'}
...
function init_gear_sets()
    ...
    sets.weapons.DaggerDW = {main="ラヌンナイフ", sub="トーレット"}
--    sets.weapons.DaggerDW = {main={ name="ロスタム", augments={'Path: A',}}, sub="トーレット"}
    sets.weapons.Sword = {main="ネイグリング", sub="ヌスクシールド"}
    sets.weapons.Dagger = {main="ラヌンナイフ", sub="ヌスクシールド"}
--    sets.weapons.Dagger = {main={ name="ロスタム", augments={'Path: A',}}, sub="ヌスクシールド"}
    sets.weapons.DeathPenalty = {range="デスペナルティ", ammo="エミネンブレット"}
    sets.weapons.Fomalhaut = {range="ハリデー", ammo="ホクスボクブレット"}
    sets.weapons.Armageddon = {range="アルマゲドン", ammo="ホクスボクブレット"}
    sets.weapons.TPBonus = {range={ name="アナーキー+2", augments={'Delay:+60','TP Bonus +1000',}}, ammo="ホクスボクブレット"}
    ...
    legs={ name="アデマケックス", augments={'AGI+10','Rng.Acc.+15','Rng.Atk.+15',}},
    feet="メガナダジャンボ+2",
    }
    ...
    waist="オネイロスセスト",
        ...
        left_ring="アルコンリング",
        right_ring="ディンジルリング",
    back={ name="ガンスリンガマント", augments={'Enmity-4','"Mag.Atk.Bns."+2','"Phantom Roll" ability delay -5','Weapon skill damage +1%',}},
    }
...
function sublimation_update()
    ...
    if buffactive['カオスロール'] then
        sublimation_box0.state = ' 　 カオス 　 '
        sublimation_box0:color(255,255,0)
        sublimation_box0:bg_color(0,0,255)
    else
        sublimation_box0.state = ' 　 カオス 　 '
        sublimation_box0:color(255,255,255)
        sublimation_box0:bg_color(255,0,0)
    end

    if buffactive['サムライロール'] then
        sublimation_box1.state = '　サムライ　'
        sublimation_box1:color(255,255,0)
        sublimation_box1:bg_color(0,0,255)
    else
        sublimation_box1.state = '　サムライ　'
        sublimation_box1:color(255,255,255)
        sublimation_box1:bg_color(255,0,0)
    end

    if buffactive['ハンターズロール'] then
        sublimation_box2.state = '　ハンター　'
        sublimation_box2:color(255,255,0)
        sublimation_box2:bg_color(0,0,255)
    else
        sublimation_box2.state = '　ハンター　'
        sublimation_box2:color(255,255,255)
        sublimation_box2:bg_color(255,0,0)
    end

    if buffactive['マイザーロール'] then
        sublimation_box3.state = '　マイザー　'
        sublimation_box3:color(255,255,0)
        sublimation_box3:bg_color(0,0,255)
    else
        sublimation_box3.state = '　マイザー　'
        sublimation_box3:color(255,255,255)
        sublimation_box3:bg_color(255,0,0)
    end

    if buffactive['タクティックロール'] then
        sublimation_box4.state = ' タクティック '
        sublimation_box4:color(255,255,0)
        sublimation_box4:bg_color(0,0,255)
    else
        sublimation_box4.state = ' タクティック '
        sublimation_box4:color(255,255,255)
        sublimation_box4:bg_color(255,0,0)
    end

    if buffactive['ナチュラリストロール'] then
        sublimation_box5.state = 'ナチュラリスト'
        sublimation_box5:color(255,255,0)
        sublimation_box5:bg_color(0,0,255)
    else
        sublimation_box5.state = 'ナチュラリスト'
        sublimation_box5:color(255,255,255)
        sublimation_box5:bg_color(255,0,0)
    end

--    if buffactive['ナチュラリストロール'] then
--        sublimation_box6.state = 'ナチュラリスト'
--        sublimation_box6:color(255,255,0)
--        sublimation_box6:bg_color(0,0,255)
--    else
--        sublimation_box6.state = 'ナチュラリスト'
--        sublimation_box6:color(255,255,255)
--        sublimation_box6:bg_color(255,0,0)
--    end

--    if buffactive['マルチアタック'] then
--        sublimation_box7.state = '　ストライ　'
--        sublimation_box7:color(255,255,0)
--        sublimation_box7:bg_color(0,0,255)
--    else
--        sublimation_box7.state = '　ストライ　'
--        sublimation_box7:color(255,255,255)
--        sublimation_box7:bg_color(255,0,0)
--    end

--    if buffactive['ブリンク'] then
--        sublimation_box8.state = ' 　ブリンク　 '
--        sublimation_box8:color(255,255,0)
--        sublimation_box8:bg_color(0,0,255)
--    else
--        sublimation_box8.state = ' 　ブリンク　 '
--        sublimation_box8:color(255,255,255)
--        sublimation_box8:bg_color(255,0,0)
--    end
...
function file_unload(file_name)
    sublimation_box0:destroy()
    sublimation_box1:destroy()
    sublimation_box2:destroy()
    sublimation_box3:destroy()
    sublimation_box4:destroy()
    sublimation_box5:destroy()
--    sublimation_box6:destroy()
--    sublimation_box7:destroy()
--    sublimation_box8:destroy()
end
